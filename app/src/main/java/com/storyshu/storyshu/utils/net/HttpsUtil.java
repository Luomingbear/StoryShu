...
public class HttpsUtil {
    ...
    /**
     * 获取api的SSLSocketFactory
     *
     * @param context
     * @return
     */
    public static SSLSocketFactory getApiSSlFactory(Context context, String name) {

        try {
            CertificateFactory cf = CertificateFactory.getInstance("X.509");
            InputStream caInput = new BufferedInputStream(context.getAssets().open(name));//把证书打包在asset文件夹中
            Certificate ca;
            try {
